...
const SupportTeam = () => {
  ...
  useEffect(() => {
    // Fetch customer data
    fetch('http://localhost:8090/teams/2')
      .then(response => response.json())
...
<div className="container ml-0">
  ...
  Axis Bank Support Team <span style={{ fontStyle: 'italic',color:'#871f40'}}>Dil se open</span>
</h2>
  ...
  <h5>{TeamData ? `${TeamData.teamName} ` : 'Your Profile'}</h5>
</div>
...
<div className="container bg-white p-5 mt-1" style={{ height: '1600px', width: '1200px', marginLeft: '40px',boxShadow: '0px 4px 8px rgba(1, 1, 1, 0.3)' }}>
  ...
  <SupportCarousel style={{ marginTop: '10px', marginLeft: '30px', marginRight: '30px',boxShadow: '0px 4px 8px rgba(1, 1, 1, 0.3)' }} />
  ...
  onChange={(e) => handleStatusChange(e.target.value, complaint.complaintId)}
>
  ...
  onClick={() => handleStatusChange(complaint.status, complaint.complaintId)}
>
  Update
</button>
...
<div className=" used cards" style={{ width:'1200px',height: '300px',marginTop:'10px' ,boxShadow: '0px 4px 8px rgba(1, 1, 1, 0.3)'}}>
<div className="row">
